 ...
 * @{
 */
/* ØªØ¹Ø±ÙŠÙ Ø³Ø¬Ù„ ØªÙ…ÙƒÙŠÙ† Ø§Ù„Ù…Ù‚Ø§Ø·Ø¹Ø§Øª NVIC_ISER0 */
#define NVIC_ISER0   (*(volatile uint32_t*)0xE000E100UL)

/* ØªØ¹Ø±ÙŠÙ Ù…ØµÙÙˆÙØ© Ø£ÙˆÙ„ÙˆÙŠØ§Øª Ø§Ù„Ù…Ù‚Ø§Ø·Ø¹Ø§Øª NVIC_IPR */
...
void HAL_CAN_MspInit(CAN_HandleTypeDef* hcan)
{
    ...
    PA12     ------> CAN_TX
		 */
		GPIO_InitStruct.GPIO_PinNumber = GPIO_PIN_11;
		GPIO_InitStruct.GPIO_Mode = GPIO_Mode_Inp_Floating;
		MCAL_GPIO_Init(GPIOA, &GPIO_InitStruct);

		GPIO_InitStruct.GPIO_PinNumber = GPIO_PIN_12;
		GPIO_InitStruct.GPIO_Mode = GPIO_Mode_Out_AF_push_pull;
...
//		/* USER CODE BEGIN CAN1_MspInit 1 */
		...
		NVIC_IPR[22] = (0 << 4);
		        NVIC_IRQ19_CAN1_TX_Enable();
		        NVIC_IRQ20_CAN1_RX0_Enable();
		        NVIC_IRQ22_CAN1_SCE_Enable();
...
//    PA12     ------> CAN_TX
//    */
//    MCAL_GPIO_DEINIT(GPIOX)(GPIOA, GPIO_PIN_11|GPIO_PIN_12);
//
...
//	 (++) HAL_CAN_GetTxMailboxesFreeLevel() to get the number of free Tx
//	                 mailboxes.
	nu_freemailbox = HAL_CAN_GetTxMailboxesFreeLevel(&hcan) ;
	if( nu_freemailbox )
	{
//		(++) HAL_CAN_AddTxMessage() to request transmission of a new
//		                 message.
...
//		(++) HAL_CAN_IsTxMessagePending() to check if a message is pending
//		                 in a Tx mailbox.
		//wait until message is transmitted(polling)
		if(polling_enable)
		{
